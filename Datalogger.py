...
class Sniffer(Thread):
    ...
    def print_packet(self, packet):
        ...
        tcpFlagString = [ch for ch in str(packet[TCP].flags)]  # example: [P, A]
        tcpFlags = []
        ...
        lengthField = packetPayload[8:12]       # 2 bytes, describes len of unitID,funcCode,data
        ...
        if(pairDict.get('MAC: '+str(csvPacket["MAC Source"])+'   IP: '+str(packet[IP].src)) == None):
            pairDict['MAC: '+str(csvPacket["MAC Source"])+'   IP: '+str(packet[IP].src)] = meanflow(client_IP=packet[IP].src, window=1)
        ...
        trueMF = pairDict['MAC: '+str(csvPacket["MAC Source"])+'   IP: '+str(packet[IP].src)].check_flow()
        ...
        protocolOverhead = float(len(packet) - len(packetPayload)) / len(packet)
        ...
        if (len(contents) % writeInterval == 0):
            ...
            for csvLine in contents:
                with open('output_packets.csv', 'a', newline='') as file_packet:
...
with open('output_packets.csv', 'w', newline='') as file_packet:
    writer_packets = csv.writer(file_packet)
    writer_packets.writerow(csvPacketHeader)

# Interface for TCP connection
#   Windows machine uses names; Linux uses enp0s3, enp0s8, eth0, etc.
interfaceName = "Ethernet"

# Declare global variables to be read from packet handling
contents = []                   # List of lists, holds each row to write to file and resets every writeInterval
dstList = []                    # List of dst IPs and time values to access
srcList = []                    # List of src IPs and time values to access
packetPayloadsList = []         # List of ALL packet payloads used in calculating Shannon entropy
...
meanFlowList = []               # list of mean flow objects for each instance of a client IP
...
thread1=threading.Thread(target=label_monitor)
thread1.start()
...
try:
    ...
    # Coils and their Headers & Offsets
    coilsHeader = []
    coilList = []
    for i in y["coils"]:
    ...
    # Registers and their Headers & Offsets
    registersHeader = []
    registerList = []
    for i in y["registers"]:
        registersHeader.append(i)
        registerList.append(y["registers"][i])
    ...
    csvRowHeader.extend(registersHeader)
    ...
    while (s < loopIterations):
        ...
        if(s % setInterval == 0):
            for vals in context:
                with open('output.csv', 'a', newline='') as file_append:
...
print("========= Metrics =========")
print("Packet Entropy [Specific_Instance/Overall] = ",Hentropy(packetPayloadsList))
...
# 3. Throughput                         ----
# 4. Channel Utilization                xxxx
# 5. Entropy of medium sized packets    x??x
# 6. Mean number of packets per flow    xxxx
# 7. Percentage of small-size packets   x??x
# 8. Percentage of large-size packets   x??x
# 9. Number of destination port         ----
# 10.Number of source port              ----
# 11.Number of peers tracked by the "IP, mac" pair      ----
# 12.Include Class label                                ----
# ------------------------------------
# Other features:-
# CPU Utilization
# RAM Utilization